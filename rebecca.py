...
giphy="https://api.giphy.com/v1/gifs/search?api_key=dc6zaTOxFJmzC&rating=r&q="
...
while bot.connected == True:
	...
	if text.find("JOIN "+channel) != -1 and cunt(text) != "rebeccablack":
			time.sleep(1)
			bot.messg(insult.greeting()+cunt(text),"g")
	if text.find("PRIVMSG "+channel) != -1:
			...
			if text.find("IS IT FRIDAY") != -1:
				...
				if (allowance < 1.0):
					...
					if cunts != "dong" or cunts != "dongerdong" or cunts != "cryptic1":
				...
				if (allowance < 1.0):
					...
					if cunts == "shoh" or cunts == "felda" or cunts == "dong" or cunts == "dongerdong" or cunts == "rebeccablack" or cunts == "stulander":
						...
						if datetime.datetime.today().weekday() == 4:
							bot.messg("Friday - Rebecca Black - Official Music Video (03:48) by rebecca -> Views: 98,992,795 | L/D: 608363/2311390 -> https://youtu.be/kfVsfOSbJY0","g")
						else:
			...
			if text.find(":.gifme ") != -1:
				...
				if (allowance < 1.0):
					...
					if cunts == "shoh" or cunts == "felda" or cunts == "dong" or cunts == "dongerdong" or cunts == "rebeccablack" or cunts == "stulander":
						...
						line = text.lower()[text.lower().index(":.gifme"):].split()
						searchterm = []
						searchterm = "+".join(line[1:]).strip()
						if searchterm:
							gif = giphy+searchterm
							gif = gif.translate(None,'!#$@%*')
							result = apiObject.getYT(gif)
							try:
								if result["data"]:
									ranum = randint(1,len(result["data"])-1)
									tellthecunts = result["data"][ranum]["images"]["fixed_width"]["url"]
									bot.messg(tellthecunts,"g")
								else:
									bot.messg("can't find your shit gif","g")
							except (IndexError, urllib2.HTTPError) as e:
								bot.messg("can't find your shit gif","g")
			...
			if text.lower().find ('f.m') != -1:
				bot.messg("https://youtu.be/g8KB45e4xow","g")
